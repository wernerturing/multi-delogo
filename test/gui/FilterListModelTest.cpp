...
public:
  FilterModelFixture()
  {
    list.insert(1, mdl::FilterListModel::filter_ptr(new fg::DelogoFilter(0, 0, 0, 0)));
    list.insert(201, mdl::FilterListModel::filter_ptr(new fg::DrawboxFilter(2, 2, 2, 2)));
    list.insert(101, mdl::FilterListModel::filter_ptr(new fg::NullFilter()));
...
BOOST_AUTO_TEST_CASE(test_get_column_type)
{
  ...
  BOOST_CHECK_EQUAL(model->get_column_type(1), Glib::Value<mdl::FilterListModel::filter_ptr>::value_type());
...
BOOST_AUTO_TEST_CASE(test_iteration)
{
  ...
  mdl::FilterListModel::filter_ptr filter0 = row0[model->columns.filter];
  ...
  mdl::FilterListModel::filter_ptr filter1 = row1[model->columns.filter];
  ...
  mdl::FilterListModel::filter_ptr filter2 = row2.get_value(model->columns.filter);
...
BOOST_AUTO_TEST_CASE(test_get_for_frame)
{
  ...
  mdl::FilterListModel::filter_ptr filter = row[model->columns.filter];
...
BOOST_AUTO_TEST_CASE(test_insert)
{
  ...
  auto returned_iter = model->insert(151, mdl::FilterListModel::filter_ptr(new fg::DelogoFilter(10, 20, 30, 40)));
...
{
  BOOST_CHECK_THROW(model->insert(101, mdl::FilterListModel::filter_ptr(new fg::DelogoFilter(10, 20, 30, 40))),
...
BOOST_AUTO_TEST_CASE(should_allow_changing_the_filter)
{
  ...
  auto new_filter = mdl::FilterListModel::filter_ptr(new fg::DrawboxFilter(10, 10, 10, 10));
  ...
  auto changed_row = *saved_iter;
  mdl::FilterListModel::filter_ptr changed_filter = changed_row[model->columns.filter];
